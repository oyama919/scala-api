  ...
  with GuiceOneAppPerSuite {
    ...
    describe("HomeController") {
      describe("route of HomeController#index") {
        it("should be valid") {
          val result: Future[Result] = route(app, FakeRequest(GET, routes.HomeController.index().toString)).get
        ...
        it("should be valid when logged in") {
          ...
          val result =
            route(
              app,
              FakeRequest(GET, routes.HomeController.index().toString).withLoggedIn(Config)(email)
            ).get
          status(result) mustBe OK
